				   ...
				   s64 link_freq)
{
	...
	unsigned long max_opp_level;
	unsigned long vote_freq;
	...
	for (i = 0; i < soc_cfg->num_clk; i++) {
		...
		if (!strcmp(clk_name, "csiphy_timer"))
			csi2phy->timer_clk_rate = round_rate;

		if (!strcmp(clk_name, "csiphy"))
...
static int phy_qcom_mipi_csi2_config_clks_nop(struct device *dev,
...
{
	const char *clk_names[] = { "csiphy", "csiphy_timer", NULL };
	unsigned int i, num_clk, num_supplies;
	struct mipi_csi2phy_device *csi2phy;
	struct dev_pm_opp_config config = {
		.clk_names = clk_names,
		.config_clks = phy_qcom_mipi_csi2_config_clks_nop,
	};
	...
	if (ret)
		return dev_err_probe(dev, ret, "apq8016 clk_enable failed\n");

	ret = devm_pm_opp_set_config(dev, &config);
	if (ret)
		return dev_err_probe(dev, ret, "Failed to set opp table config\n");
	...
	if (IS_ERR(generic_phy)) {
		...
		return dev_err_probe(dev, ret, "failed to create phy, %d\n", ret);
	}
