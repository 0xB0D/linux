...
#define csi2phy_dev_to_regs(csi2phy) &csi2phy->soc_cfg->reg_info
...
static void phy_qcom_mipi_csi2_hw_version_read(struct mipi_csi2phy_device *csi2phy)
{
	...
	hw_version = readl_relaxed(csi2phy->base +
				   CSIPHY_3PH_CMN_CSI_COMMON_STATUSn(regs->common_regs_offset, 12)) & 0xFF;
	hw_version |= readl_relaxed(csi2phy->base +
				    CSIPHY_3PH_CMN_CSI_COMMON_STATUSn(regs->common_regs_offset, 13)) << 8 & 0xFF00;
	hw_version |= readl_relaxed(csi2phy->base +
				    CSIPHY_3PH_CMN_CSI_COMMON_STATUSn(regs->common_regs_offset, 14)) << 16 & 0xFF0000;
	hw_version |= readl_relaxed(csi2phy->base +
				    CSIPHY_3PH_CMN_CSI_COMMON_STATUSn(regs->common_regs_offset, 15)) << 24 & 0xFF000000;
...
static irqreturn_t phy_qcom_mipi_csi2_isr(int irq, void *dev)
{
	...
	for (i = 0; i < CSI_COMMON_STATUS_NUM; i++) {
		int c = i + CSI_CTRL_STATUS_INDEX;
		u8 val = readl(csi2phy->base +
			       CSIPHY_3PH_CMN_CSI_COMMON_STATUSn(regs->common_regs_offset, i));

		writel(val, csi2phy->base +
		       CSIPHY_3PH_CMN_CSI_COMMON_CTRLn(regs->common_regs_offset, c));
	}

	writel(CSIPHY_3PH_CMN_CSI_COMMON_CTRL10_IRQ_CLEAR_CMD, csi2phy->base +
	       CSIPHY_3PH_CMN_CSI_COMMON_CTRLn(regs->common_regs_offset, 10));
	writel(0x0, csi2phy->base +
	       CSIPHY_3PH_CMN_CSI_COMMON_CTRLn(regs->common_regs_offset, 10));

	for (i = CSI_CTRL_STATUS_INDEX; i < CSI_CTRL_MAX; i++) {
		writel(0x0, csi2phy->base +
						 ...
						 u8 settle_cnt)
{
	...
	for (i = 0; i <= cfg->num_data_lanes; i++) {
		...
		val |= 0x17;
		writel(val, csi2phy->base + CSIPHY_3PH_LNn_CFG1(l));

		val = CSIPHY_3PH_LNn_CFG2_LP_REC_EN_INT;
		writel(val, csi2phy->base + CSIPHY_3PH_LNn_CFG2(l));

		val = settle_cnt;
		writel(val, csi2phy->base + CSIPHY_3PH_LNn_CFG3(l));

		val = CSIPHY_3PH_LNn_CFG5_T_HS_DTERM |
			CSIPHY_3PH_LNn_CFG5_HS_REC_EQ_FQ_INT;
		writel(val, csi2phy->base + CSIPHY_3PH_LNn_CFG5(l));

		val = CSIPHY_3PH_LNn_CFG6_SWI_FORCE_INIT_EXIT;
		writel(val, csi2phy->base + CSIPHY_3PH_LNn_CFG6(l));

		val = CSIPHY_3PH_LNn_CFG7_SWI_T_INIT;
		writel(val, csi2phy->base + CSIPHY_3PH_LNn_CFG7(l));

		val = CSIPHY_3PH_LNn_CFG8_SWI_SKIP_WAKEUP |
			CSIPHY_3PH_LNn_CFG8_SKEW_FILTER_ENABLE;
		writel(val, csi2phy->base + CSIPHY_3PH_LNn_CFG8(l));

		val = CSIPHY_3PH_LNn_CFG9_SWI_T_WAKEUP;
		writel(val, csi2phy->base + CSIPHY_3PH_LNn_CFG9(l));

		val = CSIPHY_3PH_LNn_TEST_IMP_HS_TERM_IMP;
		writel(val, csi2phy->base + CSIPHY_3PH_LNn_TEST_IMP(l));
	...
	writel(val, csi2phy->base + CSIPHY_3PH_LNn_CFG4(l));

	val = CSIPHY_3PH_LNn_MISC1_IS_CLKLANE;
	writel(val, csi2phy->base + CSIPHY_3PH_LNn_MISC1(l));
}
